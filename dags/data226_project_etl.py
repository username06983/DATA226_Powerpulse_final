...
def load(records: list):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # temp table for this run’s rows
            cur.execute("""
            ...
            cur.executemany(insert_tmp_sql, data)
...
with DAG(
    ...
    rows = extract(
    start="{{ data_interval_start.in_timezone('UTC').strftime('%Y-%m-%dT%H') }}",
    end="{{ data_interval_end.in_timezone('UTC').strftime('%Y-%m-%dT%H') }}",)
